...
| V3 | Legible at thumbnail scale | use the numbers in the rulebook's "Channel brand (YouTube)" rule 6, never a figure from memory (the design lead's ruling 2026-09-06 superseded the old single-line 50px / 6-word gate). As of that ruling: headline 96 to 132 px and 1 to 3 words; subline 32 to 45 px and at most 5 words, at most half the rendered headline size; word caps are PER LINE, no total cap; badge and wordmark are chrome and are neither counted nor sized against the floor. Grep the spec or the builder's sidecar for the rendered sizes, count each line's words, paste all four numbers |
| V4 | No added image on a YouTube thumbnail unless you asked for it (Owner rule 2026-09-19, rulebook Channel brand rule 3) | read the tile's sidecar: `focal` must be null, or carry an `owner_asked` string quoting her request for that image. A drawn object with no such record is a FAIL. NEVER list "add a distinct focal subject" as a fix: when a tile reads as a recolour of a neighbour, the only fixes you may name are a different tint, a different or new sky cue, or different copy. This outranks the "by focal subject" wording in the learned block below. Tiles she approved with a subject before the rule (ridge, bluff, tor) pass |
